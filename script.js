...
function agregarTableroItem(itemData = null) { // Added itemData parameter
    ...
    `;
     ...
     // Populate if itemData is provided (for editing)
     if (itemData) {
         ...
         if (itemData.tipo) { // Predefined material
             selectElement.value = itemData.tipo;
             cantidadInput.value = itemData.cantidad;
             toggleCustomMaterialInputs(selectElement, 'tablero'); // Ensure correct inputs are shown/hidden
         } else if (itemData.nombre && itemData.precio !== undefined) { // Custom material
 ...
 function agregarCantoItem(itemData = null) { // Added itemData parameter
     ...
     if (itemData) {
         ...
         if (itemData.tipo) { // Predefined material
             selectElement.value = itemData.tipo;
             cantidadInput.value = itemData.cantidad;
             toggleCustomMaterialInputs(selectElement, 'canto'); // Ensure correct inputs are shown/hidden
         } else if (itemData.nombre && itemData.precio !== undefined) { // Custom material
             selectElement.value = 'custom';
             customNombreInput.value = itemData.nombre;
             customPrecioInput.value = itemData.precio;
...
function agregarHerrajeItem(categoria, itemData = null) { // Added itemData parameter
    ...
    `;
     ...
     if (itemData) {
         ...
         if (itemData.tipo) { // Predefined material
             selectElement.value = itemData.tipo;
             cantidadInput.value = itemData.cantidad;
             toggleCustomMaterialInputs(selectElement, 'herraje'); // Ensure correct inputs are shown/hidden
         } else if (itemData.nombre && itemData.precio !== undefined) { // Custom material
             selectElement.value = 'custom';
             customNombreInput.value = itemData.nombre;
             customPrecioInput.value = itemData.precio;
...
function eliminarMueble(index) {
 ...
 // Función para editar un mueble existente
 function editarMueble(index) {
     console.log("Editando mueble en índice:", index); // Debug log
     const muebleToEdit = muebles[index];

     if (!muebleToEdit) {
         console.error("Mueble a editar no encontrado en el índice:", index);
         return;
     }

     // 1. Cargar datos del mueble en el formulario de "Configurar Mueble Actual"
     ...
     document.getElementById('costo-transporte').value = muebleToEdit.transporte || 0;
     document.getElementById('costo-instalacion').value = muebleToEdit.instalacion || 0;

     // Limpiar los contenedores dinámicos actuales
     document.querySelectorAll('.current-mueble-section .dynamic-items-container').forEach(container => {
         container.innerHTML = '';
     });

     // Rellenar los contenedores dinámicos con los materiales del mueble a editar
     muebleToEdit.tableros.forEach(tablero => agregarTableroItem(tablero));
     muebleToEdit.cantos.forEach(canto => agregarCantoItem(canto));
     // Need to add herrajes by category
     const herrajesByCategory = {
         correderas: [],
         bisagras: [],
         tiradores: [],
         otros: []
     ...
     muebleToEdit.herrajes.forEach(herraje => {
         ...
         if (herrajesByCategory[category]) {
             herrajesByCategory[category].push(herraje);
         } else {
              // Handle custom herrajes without a predefined category by adding them to 'otros'
              herrajesByCategory['otros'].push(herraje);
         }
     });
     ...
     // --- Agregar un selector vacío por defecto a cada sección después de cargar los existentes ---
     // Esto asegura que siempre haya un selector para agregar nuevos materiales/herrajes
     agregarTableroItem();
     agregarCantoItem();
     ...
     console.log("Added initial empty selectors after loading existing materials."); // Debug log
     // --------------------------------------------------------------------------------------


     // 2. Eliminar el mueble del array (el usuario lo agregará de nuevo con "Agregar Mueble")
     muebles.splice(index, 1);

     // 3. Actualizar la lista mostrada y el total
     actualizarListaMuebles();

      // Opcional: Desplazar la vista hacia el formulario de edición
     document.getElementById('nombre').scrollIntoView({ behavior: 'smooth' });

     console.log("Mueble cargado en el formulario para edición."); // Debug log
 }
...
function generarListaMateriales() {
    ...
    muebles.forEach((mueble, index) => {
        ...
        // Listar tableros usados para este mueble
        ...
        // Listar cantos usados para este mueble
        ...
        // Listar herrajes usados para este mueble
...
function formatDate(date) {
    ...
    return `${year}-${month}-${day}`; // Cambiado a-MM-DD para consistencia
}
